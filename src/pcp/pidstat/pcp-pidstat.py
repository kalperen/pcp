...
PIDSTAT_METRICS = ['kernel.uname.nodename', 'kernel.uname.release', 'kernel.uname.sysname',
                    ...
                    'proc.memory.vmstack', 'proc.psinfo.sname', 'proc.psinfo.wchan_s']
...
class PidstatOptions(pmapi.pmOptions):
    ...
    #To do: Adjust this so that -B doesn't get mixed either
    ...
    def extraOptions(self, opt,optarg, index):
        if opt == 'k':
            ...
            PidstatOptions.show_process_states = True
...
class PidstatReport(pmcc.MetricGroupPrinter):
    ...
    def report(self,manager):
        ...
        if(PidstatOptions.show_process_stack_util):
            ...
            report.print_report(timestamp, header_indentation, value_indentation, interval_in_seconds)
        else:
